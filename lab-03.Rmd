...
library("dplyr")
library("zoo")
hvi_tidy_include <- hvi_tidy %>%
  mutate(HVI = na.approx(HVI))


colSums(is.na(hvi_tidy_include))
```
...
```{r}
no_wa_summary <- nmdb %>%
  select(GEOID,PERIOD,SERIESID, VALUE1)%>%
  filter(GEOID !="WA", SERIESID != "TOT_LOANS")%>%
  summarize(
    no_wa_mean_value1 = mean(VALUE1, na.rm = TRUE),
    no_wa_median_value1 = median(VALUE1, na.rm =TRUE),
    no_wa_min_value1 = min(VALUE1, na.rm = TRUE),
    no_wa_max_value1 = max(VALUE1, na.rm = TRUE),
    no_wa_25th_percentile_value1 = quantile(VALUE1, probs = 0.25, na.rm = TRUE),
    no_wa_75th_percentile_value1 = quantile(VALUE1, probs = 0.75, na.rm = TRUE)
  )
no_wa_summary
```
...
It went from 377,440 x 14 to 377,440 x 25
...
no_wa_summary <- nmdb %>%
  ...
  filter(
    GEOID %in% c("WA","OR","CA"),
    SERIESID == "PCT_VS_VERYPOOR",
    PERIOD %in% c("2021Q1", "2021Q2", "2021Q3", "2021Q4","2022Q1", "2022Q2","2022Q3","2022Q4","2023Q1", "2023Q2","2023Q3","2023Q4", "2024Q1", "2024Q2","2024Q3","2024Q4"))

ggplot(data = no_wa_summary) +
  geom_tile(aes(x = PERIOD, y = GEOID, fill = VALUE1)) +
  scale_fill_viridis_c(option = "magma", direction = -1) +
  labs(
    title = "Percent of Very Poor Loans on the West Coast 2021 - 2024",
    x = "YEAR AND QUARTER",
    y = "STATE"
  ) +
  theme_minimal() +
...
```{r}
west_coast_summary <- nmdb %>%
  select(GEOID,PERIOD,SERIESID, VALUE1)%>%
  filter(
    GEOID %in% c("WA","OR","CA"),
    SERIESID == "TOT_LOANAMT",
    PERIOD %in% c("2021Q1", "2021Q2", "2021Q3", "2021Q4","2022Q1", "2022Q2","2022Q3","2022Q4","2023Q1", "2023Q2","2023Q3","2023Q4", "2024Q1", "2024Q2","2024Q3","2024Q4"))

ggplot(data = west_coast_summary)+
  geom_point(aes(x = PERIOD, y = GEOID, size = VALUE1 ), alpha = 0.6) + # bubble size = population
                   scale_size_continuous(range = c(1, 10), labels = scales::comma) +
                   labs(
                     title = "Bubble Chart of Total Loan Amounts on West Coast",
                     x = "State",
                     y = "Year",
                     size = "Total Loan Amounts"
                   ) +
                   theme_minimal() +
                   theme(
                     legend.position = "bottom",
...
west_coast_summary <- nmdb %>%
  ...
  filter(
    GEOID %in% c("WA","OR","CA"),
    SERIESID == "AVG_LOANAMT",
    PERIOD %in% c("2021Q1", "2021Q2", "2021Q3", "2021Q4","2022Q1", "2022Q2","2022Q3","2022Q4","2023Q1", "2023Q2","2023Q3","2023Q4", "2024Q1", "2024Q2","2024Q3","2024Q4"))


ggplot(
  data = west_coast_quality_summary,
  aes(y = VALUE1, x = GEOID)
) +
  geom_boxplot(
    fill = "black",  
    alpha = 0.3,       # transparency
  ...
  labs(
    title = "Boxplot of Average Loan Amount by State",
    y = "Average Loan Amount by Quarter",
    x = "State"
  ) +
  theme_minimal()
```
...
west_coast_summary <- nmdb %>%
  ...
  filter(
    GEOID == "WA",
    SERIESID %in% c("PCT_VS_VERYPOOR", "PCT_VS_POOR", "PCT_VS_FAIR", "PCT_VS_GOOD", "PCT_VS_EXCELLENT"),
    PERIOD %in% c("2021Q1", "2021Q2", "2021Q3", "2021Q4",
                  "2022Q1", "2022Q2", "2022Q3", "2022Q4",
                  "2023Q1", "2023Q2", "2023Q3", "2023Q4",
                  "2024Q1", "2024Q2", "2024Q3", "2024Q4")
  )

ggplot(west_coast_summary, aes(x = PERIOD, y = VALUE1, group = SERIESID, color = SERIESID)) +
  geom_area(
    alpha = 0.8,
    linetype = 6,   # 'twodash'
    linewidth = 0.8
  ...
  labs(
    title = "Washington State Quality of Loans as Percent of Overall",
    x = "Period (Quarter)",
    y = "Percent",
    color = "Series ID"
  ) +
...
In chart 1 I analyzed how very poor mortgages are are increasing over time in WA, CA, and OR. My hope was to identify how they were worsening, it showed very poor mortgages increasing in recent history which meshes with higher interest rates.

In chart 2 I loooked at total loan amounts on the west coast to see which state has biggest liabilities relative to worsening loan performance.

For chart 3 I looked at how the average loan amount varies by state over time, which shows there is higher up volatility on loan amount over time versus downside, this leads me to believe there is a relative price floor with periods of dramatically increasing loans

For chart 4 I look at percent share by loan quality in Washington which shows good and fair loans declining while there is a increase in poor and very poor.
