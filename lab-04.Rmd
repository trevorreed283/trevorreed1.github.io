...
ggplot(seattle_apartments, aes(x = month, y = RENT_PRICE, color = bed_group)) +
  geom_line(stat = "summary", fun = "mean", size = 1) +
  labs(
    title = "Average Monthly Rent Prices in Seattle (2019-2022)",
    x = "Month",
    y = "Average Rent Price",
    color = "Neighborhood"
  ) +
...
```
  I am interested in how rents performed across Seattle and in specific neghborhoods
...
seattle_neighborhoods <- seattle_apartments %>%
  filter(
    !is.na(NEIGHBORHOOD) & NEIGHBORHOOD != "",
    month >= as.Date("2019-12-01"),
    month <= as.Date("2022-12-01")
  )
#My goal is to see if there were any rent anamomalies in seattle neighborhoods during the pandemic period.
ggplot(seattle_neighborhoods, aes(x = month, y = RENT_PRICE, color = bed_group)) +
  geom_line(stat = "summary", fun = "mean", size = 1) +
  facet_wrap_paginate(~ NEIGHBORHOOD, ncol = 3, nrow = 2, page = 1) +
  labs(
    title = "Average Monthly Rent Prices in Seattle Neighborhoods (2019-2022)",
    x = "Month",
    y = "Average Rent Price",
    color = "Neighborhood"
  ) +
...
```{r} 
unique(seattle_neighborhoods$NEIGHBORHOOD)
```

I am look to see whether there are any anomalies in rental costs by neighborhood during the COOIVD period. 77 neighborhoods is too many to conduct a full analysis of so I will select 6 that are dense urban villages spread throughout the City. Queen Anne, Admiral, Northgate, Beacon Hill, Pioneer Square, Capitol Hill.

```{r}
seattle_neighborhoods_selected <- seattle_neighborhoods %>%
  filter(
    NEIGHBORHOOD %in% c("Queen Anne", "Admiral", "Northgate", "Beacon Hill", "Pioneer Square", "Capitol Hill"),
    month >= as.Date("2019-12-01"),
    month <= as.Date("2022-12-01")
  )

ggplot(seattle_neighborhoods_selected, aes(x = month, y = RENT_PRICE, color = bed_group)) +
  geom_line(stat = "summary", fun = "mean", size = 1) +
  facet_wrap_paginate(~ NEIGHBORHOOD, ncol = 3, nrow = 2, page = 1) +
  labs(
    title = "Average Monthly Rent Prices in Seattle Neighborhoods (2019-2022)",
...
```
Within these six neighborhoods, it appears there is greater volatility in very dense neighborhoods likes Pioneer Square, Capital Hill, and Queen Anne. While there is less variance further from the city in Admiral, Beacon hill and Northgate. This could be due to people avoiding the most dense areas during the pandemic.
...
```{r}
ggplot(seattle_neighborhoods_selected, aes(x = month, y = RENT_PRICE, color = NEIGHBORHOOD)) +
  geom_line(stat = "summary", fun = "mean", size = 1) +
  facet_wrap_paginate(~ bed_group, ncol = 3, nrow = 2, page = 1) +
  labs(
    title = "Average Monthly Rent Prices in Seattle Neighborhoods (2019-2022)",
    x = "Month",
    y = "Average Rent Price",
...
```
 I swapped rooms with neighborhoods. Looking at the 2 bedroom and 3 bedroom grpahs you would conclude there's no discernible trends. However, when looking at the citywide scale it is clear the rent growth for 2 bedrooms have exceeded studios and one bedrooms - it is that there are two few observations within neighborhoods to see these trends. 
 
I chose this representation to emphasize the need for appropriately sized samples to make conclusions.
