...
```{r}
kingco_homes %>%
  select(pinx,city,land_val,imp_val,sqft) %>%
  filter(city == 'BELLEVUE')%>%
  mutate(total_val = land_val + imp_val) %>%
  summarize(
...
kingco_homes %>%
  ...
  mutate(
    view_likes = ifelse(view_lakewash == 1 & view_lakesamm == 1, 1, 0))%>%
  filter(city == 'BELLEVUE',view_likes == 1) %>%
  mutate(total_val = land_val + imp_val) %>%
  summarize(
    mean_total_val = mean(total_val,na.rm = TRUE),
    var_total_val = var(total_val,na.rm = TRUE))
...
The difference in mean between view properties and non view properties in Bellevue is
...
```{r}
colnames(kingco_homes_clean)
```
```{r}
kingco_homes_clean %>%
  select(city,land_val,imp_val,sqft)%>%
  filter(city == 'SEATTLE') %>%
  summarize(
    value_sqft = (sum(land_val+imp_val,na.rm = TRUE)/sum(sqft, na.rm = TRUE)))

```
```{r}
kingco_homes_clean %>%
  select(pinx)%>%
  count(pinx,na.rm = TRUE)
```
...
abline(v = sale_price_mean, col = "red", lwd = 2)
```
The normal distribuition is less smooth due to the smaller sampling size
...
kingco_homes_clean %>%
  select(city, pinx,view_rainier,view_olympics,view_cascades,view_territorial,view_skyline,view_sound,view_lakewash,view_lakesamm,land_val,sqft_lot)%>%
  filter(city == 'BELLEVUE')%>%
  mutate(view_water = ifelse(view_lakewash == 1 & view_lakesamm == 1 & view_sound == 1, 1, 0))%>%
  filter(view_water == 1)%>%
  summarise(average_sqft_sale_price_water = sum(land_val,na.rm = TRUE)/sum(sqft_lot, na.rm =TRUE))%>%
  rename(waterfront_sqft_cost_bellevue = average_sqft_sale_price_water)
